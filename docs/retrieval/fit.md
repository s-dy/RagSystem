...
> **文档说明**：本文档记录了 HybridRAG 系统检索阶段的问题及修复状态。
>
> **最后更新**：2026-06-07  
> **当前状态**：大部分高优先级问题已修复，多模态检索已实现
...
| 问题 | 优先级 | 状态 | 修复方案 |
|------|--------|------|----------|
| DocumentGrader 整体评分 | P0 | ✅ 已完成 | 改为逐文档 embedding 相似度评分 |
| 重排序后无兜底机制 | P0 | ✅ 已完成 | 全部过滤时保留 top-1 |
| 融合去重策略粗暴 | P1 | ✅ 已完成 | embedding 余弦相似度 0.92 阈值近似去重 |
| CrossEncoder max_length=512 | P1 | ⚠️ 部分修复 | 长文档截断，但可接受 |
| 向量检索固定 k=4 | P1 | ⚠️ 待优化 | 可根据任务类型动态调整 |
| 查询增强缺乏质量控制 | P2 | ⚠️ 待优化 | 已有语义去重，可加强质量评估 |
| 查询路由无缓存和回退 | P2 | ✅ 已完成 | LLM 路由失败时回退到全库检索 |
| 相似度阈值硬编码 | P2 | ⚠️ 待优化 | 提取为配置项，支持动态调整 |
| 图片检索无并行化 | P1 | ✅ 已完成 | asyncio.gather 并行检索所有 collection |
| 图片检索无阈值过滤 | P1 | ✅ 已完成 | IMAGE_SCORE_THRESHOLD=0.25 过滤 |
| 图文 RRF 融合缺失 | P1 | ✅ 已完成 | CLIP score 归一化后与文字 rerank score 对齐 |
| 表格内容检索召回率低 | P2 | ✅ 已完成 | Markdown + 自然语言摘要双格式 |

### ⚠️ 待优化的功能

| 优化项 | 优先级 | 影响范围 | 建议方案 |
|--------|--------|----------|----------|
| 内部/外部检索并行化 | P1 | 检索效率 | `_retrieve_internal` 和 `_retrieve_external` 改为 `asyncio.gather()` |
| 动态 k 值调整 | P1 | 检索质量 | 根据 TaskType 动态调整 k（简单查询 k=2，复杂查询 k=8） |
| 语义去重 + MMR | P2 | 结果多样性 | embedding 相似度 > 0.95 合并，重排序后 MMR |
| 查询增强质量评估 | P2 | 检索质量 | LLM 评估增强结果质量，低质量改写丢弃 |
| 动态阈值调整 | P2 | 检索适应性 | 根据任务类型和 embedding 分布动态调整阈值 |
| 元数据过滤 | P3 | 检索精度 | Milvus 检索支持 metadata filter 表达式 |
| 外部检索改写复用 | P3 | 效率优化 | 复用 QueryEnhancer 的改写结果，避免重复 LLM 调用 |
| grade 与重排序统一 | P3 | 系统简化 | 统一评分标准，或 grade 失败时调整增强策略 |
