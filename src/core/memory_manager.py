...
class MemoryManager:
    ...
    async def get_recent_conversation_memories(
        ...
        results = [
            item.value for item in items if item.value and isinstance(item.value, dict)
        ]
        results.sort(key=lambda x: x.get("timestamp", 0), reverse=True)
        logger.debug(
            f"[MemoryManager] 获取对话记忆: user={user_id}, thread={thread_id}, type={memory_type}, count={len(results[:limit])}"
        )
