...
class EnhancedTimeParser:
    ...
    def _compile_patterns(self):
        ...
        self.patterns = {
            ...
            # 带前后缀的时间表达式
            "prefix_time": re.compile(
                r"(前|后|上|下|本|这)(个?)(天|日|周|星期|月|季度|年)"
            ),
            # 时间段
            "time_period": re.compile(
                r"(凌晨|早晨|早上|上午|中午|下午|傍晚|晚上|深夜|午夜)"
            ),
            # 星期几
            ...
            # 简单数字+N天/周/月/年
            "simple_recent_future": re.compile(
                r"([一二三四五六七八九十\d]+)(天|周|月|季度|年)(内|前|后|之前|之后)?"
            ),
        }

    def parse(
        self, text: str, time_base: datetime = None, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        解析文本中的时间表达式
        ...
        try:
            ...
            pass  # jionlp 解析失败时静默跳过，尝试其他解析方式
        ...
        logger.debug(f"[TimeParser] 解析结果: {results}")
        return results

    def _parse_with_jionlp(
        self, text: str, time_base: datetime, config: Dict
    ) -> List[Dict[str, Any]]:
        """使用jionlp解析时间"""
        results = []
    ...
    def _parse_standard_formats(
        self, text: str, time_base: datetime
    ) -> List[Dict[str, Any]]:
        """解析标准日期格式"""
        results = []
            ...
            try:
                ...
                results.append(
                    self._create_time_span(
                        dt, dt + timedelta(days=1), "accurate", "std_ymd"
                    )
                )
            except ValueError:
                pass
        ...
        if match:
            ...
            try:
                ...
                results.append(
                    self._create_time_span(
                        dt, dt + timedelta(days=1), "accurate", "std_md"
                    )
                )
            except ValueError:
                pass
    ...
    def _parse_relative_time(
        ...
        if "后天" in text:
            ...
            results.append(
                self._create_time_span(start, end, "accurate", "day_after_tomorrow")
            )
    ...
    def _parse_time_periods(
        ...
        for period, (start_hour, end_hour) in period_map.items():
            if period in text:
                ...
                results.append(
                    self._create_time_span(
                        start_time, end_time, "accurate", f"period_{period}"
                    )
                )

        return results

    def _parse_special_expressions(
        self, text: str, time_base: datetime
    ) -> List[Dict[str, Any]]:
        """解析特殊表达式"""
        results = []
        ...
        if match:
            ...
            if prefix in ["最近", "过去", "以往"]:
                ...
                results.append(
                    self._create_time_span(
                        start, end, "accurate", f"recent_{num}{unit}"
                    )
                )
                ...
                results.append(
                    self._create_time_span(
                        start, end, "accurate", f"future_{num}{unit}"
                    )
                )
        ...
        if match:
            ...
            if season in season_map:
                ...
                results.append(
                    self._create_time_span(
                        start_date, end_date, "accurate", f"season_{season}"
                    )
                )
        ...
        for pattern, (start_month, end_month) in quarter_patterns.items():
            if pattern in text:
                ...
                results.append(
                    self._create_time_span(
                        start_date, end_date, "accurate", f"quarter_{pattern}"
                    )
                )

        return results

    def _parse_weekday_expressions(
        self, text: str, time_base: datetime
    ) -> List[Dict[str, Any]]:
        """解析星期几表达式"""
        results = []
        ...
        if match:
            ...
            if weekday_str in weekday_map:
                ...
                results.append(
                    self._create_time_span(
                        start, end, "accurate", f"weekday_{weekday_str}"
                    )
                )
        ...
        if match:
            ...
            results.append(
                self._create_time_span(
                    start, end, "accurate", f"weekday_num_{weekday_num + 1}"
                )
            )

        return results

    def _parse_simple_relative_time(
        self, text: str, time_base: datetime
    ) -> List[Dict[str, Any]]:
        """解析简单相对时间（如"三天内"）"""
        results = []
        ...
        if match:
            ...
            if suffix in ["内", "之后", "后"]:
                ...
                results.append(
                    self._create_time_span(start, end, "accurate", f"past_{num}{unit}")
                )
            else:
                # 没有后缀，默认是过去
                end = time_base
    ...
    def _create_time_span(
        self, start: datetime, end: datetime, definition: str, source: str
    ) -> Dict[str, Any]:
        """创建标准的时间范围字典"""
        return {
            "type": "time_span",
    ...
    def _merge_and_sort_results(self, results: List[Dict], config: Dict) -> List[Dict]:
        ...
        if config.get("strict_mode"):
            unique_results = [
                r for r in unique_results if r["definition"] == "accurate"
            ]
