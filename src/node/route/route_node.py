...
class RouteNodeMixin:
    ...
    async def __retrieve_or_respond(
        ...
        if self.config.enable_conversation_compress:
            ...
            logger.debug(
                f"[RouteNode] 对话历史检查: turns={current_turns}, tokens={current_tokens}, turns_exceeded={turns_exceeded}, tokens_exceeded={tokens_exceeded}"
            )
            if turns_exceeded or tokens_exceeded:
                logger.info("[RouteNode] 触发对话历史压缩")
        ...
        # === 策略 6：渐进式摘要 ===
        if should_trigger_incremental_summary(
            messages, self.config.incremental_summary_interval
        ):
        ...
        logger.info(
            f"[RouteNode] 任务分析: type={task_char.task_type.value}, is_multi_hop={task_char.is_multi_hop}, requires_external={task_char.requires_external_tools}"
        )

        # 检查是否需要检索
        is_need_retrieval = await check_need_retrieval(
            self.llm, query, conversation_context
        )
        is_need_retrieval = (
            is_need_retrieval or task_char.task_type == TaskType.FACT_RETRIEVAL
        )

        if not is_need_retrieval:
            logger.info(f"[RouteNode] 直接对话模式: query={query[:50]}...")
            return {
                **compress_result,
    ...
    async def __prepare_next_step(
        ...
        if not state["sub_questions"]:
            logger.info("[RouteNode] 开始多跳问题分解")
            # 只开启子问题分解
    ...
    async def __enhance_and_route_current(
        self, state, config: RunnableConfig, store: BaseStore
    ) -> dict:
        """增强当前查询"""
        # 获取当前查询和上下文
    ...
    def __get_enhancer_config_by_task(
        self, task_char: TaskCharacteristics, run_count: int
